...
local function GetVaultOffsetX()
    return (NX.Constants and NX.Constants.VAULT_OFFSET_X) or 0
end
...
local flash = text:CreateAnimationGroup()
...
local function ApplyFontDeferred(path, size)
    ...
    local function try()
        tries = tries + 1
        if text:SetFont(path, size, OUTLINE) then
            return
        end
        if tries < 5 then
            C_Timer.After(0, try)
...
function V:ApplyConfig()
    local db = NX.DB and NX.DB.vault
    if not db then return end

    local fontPath = db.fontPath or "Fonts\\FRIZQT__.TTF"
    local fontSize = tonumber(db.fontSize) or 48
    local ox = GetVaultOffsetX()
    local oy = tonumber(db.offsetY) or 0

    text:ClearAllPoints()
    text:SetPoint(ANCHOR, UIParent, ANCHOR, ox, oy)
...
function V:TestPreview()
    if not (NX.DB and NX.DB.vault) then return end
    if not NX.DB.vault.enabled then
        self:HidePreview()
        return
    end
    ...
    text:SetText(BuildBannerText())
    text:SetAlpha(1)
    text:Show()

    if NX.DB.vault.flashing then
        if not flash:IsPlaying() then flash:Play() end
    else
...
function V:ShowBannerFor(seconds)
    if not (NX.DB and NX.DB.vault and NX.DB.vault.enabled) then return end
...
function V:Init()
    ...
    f:SetScript("OnEvent", function(_, event, ...)
        ...
        if event == "UNIT_SPELLCAST_SENT" then
            ...
            if not NX.DB.vault.enabled then return end
